...
const LiveBanner = ({last_price, locked_price, prize_pool, growth}: Props) => {
  return (
    <div className='bg-white mx-4 w-80 h-52 border-[3px] rounded-3xl m-0 border-[#ff69b4] flex flex-col justify-between items-start p-5' >
      <p className='text-gray-500 font-bold '  >LAST PRICE</p>
      <div className=' w-full flex justify-between items-center ' >
        <h2 className='underline  decoration-dotted decoration-black	 text-3xl font-bold text-[#ff69b4]' >${last_price}</h2>
        <button className='flex items-center gap-1 text-white p-2 rounded bg-[#ff69b4]' >
          <Image src= {arrow} height={15} alt="arrow"/> ${growth}</button>
      </div>
